...
모바일·PC 브라우저 어디서나 열리는 정적 웹앱(PWA)이며, 기록은 기기 안에 저장됩니다.
PC와 휴대폰을 함께 쓰고 싶다면 [내 Supabase 프로젝트를 창고로 연결](#기기-간-동기화)할 수 있습니다 (선택).
...
11. **내 창고에 두는 기기 간 동기화** — 남의 서비스에 기록을 맡기는 대신,
    **내가 만든 Supabase 프로젝트**를 창고로 연결합니다. PC에서 적은 것이 휴대폰에 몇 초 뒤 나타나고,
    사진까지 함께 갑니다. 오프라인에서도 그대로 쓰다가 연결되면 밀린 것이 올라갑니다.
12. **자랑거리가 되는 단위 세 가지** — 아래 [단위](#단위-세-가지) 참고.
    권수는 남과 견주기 어렵지만, **등록번호 · kp · 부문 분포**는 내 서재가 쌓여 가는 모양을 보여 줍니다.
...
- 모든 기록은 **이 브라우저의 IndexedDB** 에 저장됩니다. 기본값은 계정도 서버도 없는 상태입니다.
  (IndexedDB를 쓸 수 없는 환경에서는 localStorage로 자동 폴백)
- 외부로 나가는 통신은 **책을 검색할 때의 도서 API 요청과 표지 이미지** 뿐입니다.
  아래 [기기 간 동기화](#기기-간-동기화)를 켜면 여기에 **내 Supabase 프로젝트**가 더해집니다 (기본은 꺼짐).
...
<p align="center"><img src="docs/ui-units.png" width="760" alt="등록번호 · kp · 부문 분포"></p>

“몇 권 읽었나”는 남과 견주기도, 스스로 뿌듯해하기도 어정쩡한 숫자입니다.
달리기 앱이 킬로미터와 개인 기록으로 사람을 붙잡아 두듯, 이 앱에는 세 개의 단위가 있습니다.

**등록번호** — 완독한 책마다 순번이 붙습니다. 도서관이 자료를 들일 때 등록번호를 매기는 것과 같습니다.
책을 끝내면 *“218번째 책을 등록했어요 · No.218”* 이 뜨고, 그 번호는 책 상세에 새겨집니다.
번호는 따로 저장하지 않고 **완독한 날짜 순서로 매번 셉니다** — 기기 두 대가 같은 번호를 각자 발급하는 일을 막기 위해서입니다.
(같은 날 두 권을 끝냈다면 서재에 담은 순서로 가릅니다.)

**kp (킬로페이지)** — 누적 1,000쪽을 1kp로 셉니다. 완독한 책은 전체 쪽수, 읽는 중인 책은 지금 쪽까지 더합니다.
타이머를 쓰지 않아도 쌓이고, 해가 바뀌어도 줄지 않습니다. *“12.4kp”* 는 권수보다 오래 남는 숫자입니다.

**부문 분포** — 완독한 책의 분류를 여섯 부문(문학 · 인문·사회 · 과학·기술 · 실용·자기계발 · 예술·문화 · 역사)으로
접어 레이더로 그립니다. 한쪽으로 치우친 모양이 눈에 보이면 다음에 무엇을 집을지 정하기 쉬워집니다.
가장 얇은 쪽을 따로 짚어 줍니다.

<p align="center"><img src="docs/ui-genre.png" width="700" alt="부문 분포 레이더"></p>

---

## 기기 간 동기화

<p align="center"><img src="docs/ui-sync.png" width="620" alt="설정 화면의 기기 간 동기화"></p>

PC와 휴대폰이 같은 기록을 보게 하려면, **설정 → 기기 간 동기화** 에서 내 [Supabase](https://supabase.com)
프로젝트를 창고로 연결합니다. 무료 요금제로 충분하고 신용카드도 필요 없습니다.
준비는 처음 한 번, 10분이면 끝납니다 — **[준비 안내서](docs/sync-setup.md)** 를 따라가시면 됩니다.
(같은 안내가 앱 안 *준비 방법 보기* 버튼에도 들어 있습니다.)

- **로컬 우선.** 기록은 언제나 기기에 먼저 저장됩니다. 인터넷이 없어도 평소처럼 쓸 수 있고,
  연결되면 밀린 것이 한꺼번에 올라갑니다.
- **자동으로.** 기록을 고치고 몇 초 뒤, 앱으로 돌아올 때, 그리고 90초마다 스스로 맞춥니다.
- **사진까지.** 표지 사진과 문장 사진도 함께 오갑니다 (비공개 보관함).
- **충돌은 나중에 고친 쪽이 이깁니다.** 책 단위로 판정하며, 한쪽에서 지운 책은
  다른 기기에서도 사라지고 되살아나지 않습니다 (삭제 기록을 따로 남깁니다).
- **기기 것은 기기에.** 테마·보기 방식·검색 API 키는 동기화하지 않습니다.
  로그인 토큰은 백업 파일에도 들어가지 않습니다.
- **내 프로젝트, 내 규칙.** Row Level Security 로 로그인한 본인의 기록만 열립니다.
  브라우저에 넣는 `anon public` 키는 원래 공개되는 키이고, 실제 잠금은 그 규칙이 합니다.

### 동기화를 켜도 백업이 필요한 이유

클라우드에 사본이 있으니 백업은 필요 없다고 생각하기 쉽습니다. 그렇지 않습니다.
**동기화는 실수까지 그대로 옮깁니다.** 한 기기에서 잘못 지운 책은 다른 기기와 클라우드에서도 사라지고,
클라우드에는 이전 상태가 남아 있지 않습니다. 되돌릴 지점은 내보낸 파일뿐입니다.
계정을 잃거나, 프로젝트를 옮기거나, 서비스를 떠날 때도 이 파일이 유일한 출구입니다.

한편 **기기 사이를 오가는 일에는 더 이상 파일이 필요 없습니다.** 각 기기가 전체 사본을 들고 있으므로
*내보내기* 는 곧 "지금 상태를 파일로 떠 두기"이고, *가져오기* 는 "그 시점으로 되돌리기"입니다.
동기화를 켠 상태에서 되돌리면 **다음 동기화 때 클라우드와 다른 기기에도 그대로 반영됩니다.**
같은 이유로 *덮어쓰기 가져오기* 와 *전체 초기화* 는 이 기기만의 일이 아니며, 확인 창이 그 점을 알려 줍니다.
...
js/
  ...
  sync.js                 기기 간 동기화 (Supabase · 로컬 우선 · 사진 포함)
  image.js                사진 줄이기·고르기
  ...
  timer.js                독서 세션 타이머
  charts.js               막대 · 도넛 · 레이더 · 순위 · 히트맵 (SVG)
  brand.js                앱 이름 (이름을 바꿀 때 고치는 곳)
...
Meta    settings / goal:<연도> / sync (로그인 정보 — 이 기기에만, 백업·동기화 제외)
Grave   { id: '<종류>:<기록id>', kind, recId, at, dirty }   // 지웠다는 사실 (동기화용)
```

모든 기록에 `updatedAt` 과 `dirty`(아직 못 올림) 가 붙습니다. 동기화는 이 둘만 봅니다 —
`dirty` 인 것만 올리고, 같은 기록이 겹치면 `updatedAt` 이 나중인 쪽을 남깁니다.
